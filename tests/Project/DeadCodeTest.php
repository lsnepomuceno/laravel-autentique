 ...
 * property only ever written. **A local variable assigned and never read is
 * the one it does not see**, and no tool in the ecosystem fits, as measured in
 * laravel-a1-pdf-sign, where this walker was written:
 *
 * - `shipmonk/phpstan-rules` has no such rule.
 * - `phpmd/phpmd` could not run beside the installed Symfony.
 * - `slevomat/coding-standard` has the sniff and arrives through PHPCS, a
 *   second toolchain beside Pint for one check.
 *
 * So it is written here, walking the tree with `token_get_all()` the way
 * `tests/Project/ArchTest.php` and `tests/Project/SpecTest.php` already do.
 *
...
/**
 * A temporary file for a fixture the walker is asserted on.
 *
 * Outside the package, so a failing test that never reaches its `unlink()`
 * leaves nothing behind for the walker above to find on the next run.
 */
function deadCodeFixture(): string
{
    $path = tempnam(sys_get_temp_dir(), 'dead-code-');
...
it('does not read a parameter default as an assignment', function () {
    // The shape that produced five false positives in laravel-a1-pdf-sign: an
    // anonymous class inside a closure, whose methods carry defaults. The
